...
Inflation_Unemploymt_df["Unemployment Rate Normalized"] = (
    Inflation_Unemploymt_df["Unemployment Rate"]
    - Inflation_Unemploymt_df["Unemployment Rate"].min()
) / (
    Inflation_Unemploymt_df["Unemployment Rate"].max()
    - Inflation_Unemploymt_df["Unemployment Rate"].min()
)

Inflation_Unemploymt_df["Annual Inflation Rate Normalized"] = (
    Inflation_Unemploymt_df["Annual Inflation Rate"]
    - Inflation_Unemploymt_df["Annual Inflation Rate"].min()
) / (
    Inflation_Unemploymt_df["Annual Inflation Rate"].max()
    - Inflation_Unemploymt_df["Annual Inflation Rate"].min()
)


# Scatter plots of Annual Inflation rates and Unemployment rates 
...
Inf_Unem = Inflation_Unemploymt_df2[
    ["Unemployment Rate Normalized", "Annual Inflation Rate Normalized"]
].to_numpy()
...
# Hypothesis Testing
# Null hypothesis: the time interval between macroeconomic cluster changes
# is of exponential(if we treat time as continuous) or 
...
# Get an exponential random variable and a geometric random variable based
# on parameters derived from data
