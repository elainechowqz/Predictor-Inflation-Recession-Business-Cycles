...
Unemployment_df = pd.read_excel(
    r'/Users/Elaine Qiao Zhou/data/UNRATE.xls', skiprows=10)
Unemployment_df.columns = ['observation_date', 'Unemployment Rate']

CPI_df['Annual Inflation Rate'] = (
    (CPI_df['CPI']/CPI_df['CPI'].shift(12)) - 1)*100

Inflation_Unemploymt_df = pd.merge(
    CPI_df[12:], Unemployment_df, on='observation_date')

# Normalize data

Inflation_Unemploymt_df['Unemployment Rate Normalized'] = (Inflation_Unemploymt_df['Unemployment Rate'] - Inflation_Unemploymt_df['Unemployment Rate'].min())/(
    Inflation_Unemploymt_df['Unemployment Rate'].max() - Inflation_Unemploymt_df['Unemployment Rate'].min())

Inflation_Unemploymt_df['Annual Inflation Rate Normalized'] = (Inflation_Unemploymt_df['Annual Inflation Rate'] - Inflation_Unemploymt_df['Annual Inflation Rate'].min())/(
    Inflation_Unemploymt_df['Annual Inflation Rate'].max() - Inflation_Unemploymt_df['Annual Inflation Rate'].min())


# Scatter plots of Annual Inflation rates and Unemployment rates during different time periods

Inflation_Unemploymt_df.plot.scatter(
    x='Unemployment Rate', y='Annual Inflation Rate')


def period_scatter_plot(d1, d2, df):
    new_df = df.loc[(df['observation_date'] >= d1) &
                    (df['observation_date'] <= d2)]
    new_df.plot.scatter(x='Unemployment Rate', y='Annual Inflation Rate')


period_list = [['1949-01-01', '1959-12-01'], ['1960-01-01', '1969-12-01'], ['1970-01-01', '1979-12-01'],
               ['1980-01-01', '1989-12-01'], ['1990-01-01', '1999-12-01'], ['2000-01-01', '2009-12-01'], ['2010-01-01', '2020-10-01']]

for i in range(len(period_list)):
    period_scatter_plot(period_list[i][0],
                        period_list[i][1], Inflation_Unemploymt_df)
...
Inf_Unem = Inflation_Unemploymt_df2[[
    'Unemployment Rate Normalized', 'Annual Inflation Rate Normalized']].to_numpy()
...
color_list = ['green', 'red', 'yellow', 'purple',
              'pink', 'orange', 'cyan', 'lightgreen', 'olive']

plt.figure()

for i in range(num):
    plt.scatter(Inf_Unem[l_km == i, 0], Inf_Unem[l_km ==
                                                 i, 1], s=50, c=color_list[i], label=i)

# Plot all the centroids

plt.scatter(c_km[:, 0], c_km[:, 1], s=250,
            c='blue', marker='*', label='centroids')
...
Inflation_Unemploymt_df2['cluster change'] = Inflation_Unemploymt_df2['cluster'] - \
...
plt.figure()
plt.hist(cluster_time_intervals_months, bins=10,
         label='Cluster Time Intervals Months')
plt.legend()
...
for i in range(1, len(bins) - 1):
    expectation1 = (rv.cdf(bins[i]) - rv.cdf(bins[i - 1])
                    )*len(cluster_time_intervals_months)
    expectations1.append(expectation1)
    expectation2 = (rv2.cdf(bins[i]) - rv2.cdf(bins[i - 1])
                    )*len(cluster_time_intervals_months)
