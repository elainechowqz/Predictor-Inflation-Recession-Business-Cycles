...
price["normalized monthlylog"] = (
    price["monthly_log_return"] - monthlylog_mean
) / monthlylog_std
...
mp = monthly_price.reset_index().rename(
    columns={
...
mpa = monthly_price_avg.reset_index().rename(
    columns={
        "Date": "observation_date",
...
for j in range(len(coef_list)):
    ...
    cluster_df["prediction"] = (
        w_0 + w_1 * cluster_df.iloc[:, 3] + w_2 * cluster_df.iloc[:, 6]
    )
...
for i in range(num):
    plt.scatter(
        macsto[macro_stocks_df["cluster"] == i, 0],
        macsto[macro_stocks_df["cluster"] == i, 1],
        s=50,
        c=color_list[i],
    )
...
for i in range(num):
    plt.scatter(
        macsto[macro_stocks_df["cluster"] == i, 0],
        macsto[macro_stocks_df["cluster"] == i, 2],
        s=50,
        c=color_list[i],
    )
...
for c in cluster_list:
    monthly_return_range = [c["monthly_return"].min(), c["monthly_return"].max()]
    average_monthly_return_range = [
        c["avg_monthly_return"].min(),
        c["avg_monthly_return"].max(),
    ]
    prediction_range_list.append((monthly_return_range, average_monthly_return_range))
...
stock_prediction_from_macro_clustering(ts2, cluster_list, prediction_range_list)
